gwglmnet.cv.f = function(formula, data, weights, family, bw, coords, gweight, env, oracle, varselect.method, verbose, longlat, tol.loc, bw.type, N, parallel, interact, bwselect.method, resid.type) {    
  ...
  gwglmnet.model = gwglmnet(formula=formula, data=data, family=family,
    weights=weights, tuning=TRUE, coords=coords, gweight=gweight,
    oracle=oracle, bw=bw, N=N, varselect.method=varselect.method, verbose=verbose,
    longlat=longlat, bw.type=bw.type, parallel=parallel,
    interact=interact, tol.loc=tol.loc, resid.type=resid.type)
  ...
  if (bwselect.method=='AICc') {
    ...
    trH = sum(sapply(gwglmnet.model[['model']][['models']], function(x) {
      s2 = x[['tunelist']][['s2']]
    ...
    loss = trH + sum(sapply(gwglmnet.model[['model']][['models']], function(x) {min(x[['tunelist']][['ssr-loc']][[resid.type]])}))
