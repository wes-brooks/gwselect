gwglmnet.fit.nenparallel = function(x, y, family, coords, indx, fit.loc, D, mode.select, verbose, prior.weights, gweight, target, beta1, beta2, tol.loc, longlat=FALSE, tuning, simulation, predict, N, oracle, interact, alpha, shrunk.fit, resid.type) {
    ...
    models = foreach(i=1:n, .packages=c('glmnet'), .errorhandling='remove') %dopar% {
        ...
        opt = optimize(gwglmnet.ssr, lower=beta1, upper=beta2, 
            maximum=FALSE, tol=tol.loc, x=x, y=y, coords=coords, loc=loc, alpha=alpha, resid.type=resid.type,
        ...
        if (is.null(oracle)) {
            m = gwglmnet.fit.inner(x=x, y=y, family=family, coords=coords, loc=loc, bw=bandwidth, dist=dist, mode.select=mode.select, verbose=verbose, gwr.weights=NULL, prior.weights=prior.weights, gweight=gweight, tuning=tuning, simulation=simulation, predict=predict, N=N, interact=interact, shrunk.fit=shrunk.fit, alpha=alpha)
        } else {
