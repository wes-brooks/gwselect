gwglmnet.fit.nenparallel = function(x, y, family, coords, fit.loc, D, mode.select, verbose, prior.weights, gweight, target, beta1, beta2, tol.loc, longlat=FALSE, tuning, simulation, predict, N, oracle, interact, resid.type) {
  ...
  models = foreach(i=1:n, .packages=c('SGL'), .errorhandling='remove') %dopar% {
    ...
    #Fit the local model:
    if (is.null(oracle)) {
      m = gwglmnet.fit.inner(x=x, y=y, family=family, coords=coords, loc=loc, N=N, mode.select=mode.select, verbose=verbose, gwr.weights=kernweights, prior.weights=prior.weights, tuning=tuning, predict=predict, simulation=simulation, interact=interact)
    } else {
      m = gwselect.fit.oracle(x=x, y=y, family=family, coords=coords, loc=loc, N=N, mode.select=mode.select, verbose=verbose, gwr.weights=kernweights, prior.weights=prior.weights, tuning=tuning, predict=predict, simulation=simulation, interact=interact, oracle=oracle[[i]])
    }
